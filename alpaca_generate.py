...
def parse_args():
    ...
    parser.add_argument('--range',
                        default='0-805', type=parse_range, help="Range to process in the format start-end (e.g., 500-600) for alpaca eval dataset"
                        )
...
def main():
    ...
    if args.range:
        ...
        print(f"Processing example {start} to {end} !")
    else:
        start = 0
        end = 805
    ...
    print(start, end)
    eval_set = eval_set.select(range(start, min(end, len(eval_set))))
    if method == 'best_of_n':
