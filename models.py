...
class Darwin:
    def __init__(self,
        ...
        if 'Llama-3' in model_name:
            # Somehow loading the llama3 tokenizer from simpo checkpoints causes error, hence we load the default llama3 tokenizer
    ...
    def mutate(self, previous_instruction: str, n: int = 1):
        ...
        generated_strings = [self.tokenizer.decode(
            output_ids[i][input_len:], skip_special_tokens=True) for i in range(n)]
        # generated_string = self.tokenizer.decode(output_ids[input_len:])
